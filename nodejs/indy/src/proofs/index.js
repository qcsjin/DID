...
exports.getProofRequests = async function(force) {
    if(force || !proofRequests) {
        ...
        let MedicalrecordCredDef = await indy.issuer.getCredDefByTag('My income proof');
        ...
        let HousingCredDef = await indy.issuer.getCredDefByTag('My house certificate');
        if(HousingCredDef) {
            proofRequests['房产证明'] = {
                name: '房产证明',
                version: '0.3',
                requested_attributes: {
                    'attr1_referent': {
                        'name': '姓名',
                        'restrictions': [{'cred_def_id': HousingCredDef.id}]
                    },
                    'attr2_referent': {
                        'name': '家庭住址',
                        'restrictions': [{'cred_def_id': HousingCredDef.id}]
        ...
        let PropertyCredDef = await indy.issuer.getCredDefByTag('My property certificate');
        if(PropertyCredDef) {
            proofRequests['家庭资产证明'] = {
                name: '家庭资产证明',
                version: '0.5',
                requested_attributes: {
                    'attr1_referent': {
